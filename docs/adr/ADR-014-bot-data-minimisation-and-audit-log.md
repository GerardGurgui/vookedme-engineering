...
- `AppointmentEvent.java` *(published — SC-2)* — the mutation event; `triggeredBy` and `actorUserId` captured at publication time as explicit parameters (not thread-local); the event design is what makes the listener path attribution reliable across synchronous and batch processing paths
- `SourceActor.java` *(published — SC-1)* — the actor type enum used in event publishing and in the audit log schema; covers BOT, CUSTOMER, OWNER, ADMIN, EMPLOYEE, SYSTEM, SCHEDULER
- `AppointmentAuditFlowIT.java` *(published — SC-2)* — verifies the data minimisation invariant: `actorUserId` NULL for CUSTOMER and SCHEDULER attribution paths; `detail` contains `paid_toggle` flag for PAID_TOGGLED events; no reason content stored
- `BotPhoneMaskingService.java` *(published — SC-3)* — pure utility that masks E.164 phone numbers to `+CC *** *** <last 3>` format for list-view display; masking is applied at the backend service layer so the raw phone number is never present in the audit feed network response; the known-prefix table covers Spain (+34), North America (+1), UK (+44), France (+33), Germany (+49); the generic 2-digit fallback is explicitly documented as a deliberate simplification, not an oversight
- `BotPhoneMaskingServiceTest.java` *(published — SC-3)* — verifies Spain pilot masking, four additional known prefixes, generic fallback, rejection paths (null, blank, no-plus, too-short), and purity contract
- `WebhookAppointmentResponse.java` — the slim boundary DTO; minimised field set returned by all bot-facing webhook endpoints *(published in a subsequent source batch)*
- `WebhookController.java` — maps from the internal full DTO to `WebhookAppointmentResponse` at the response boundary *(published in a subsequent source batch)*
